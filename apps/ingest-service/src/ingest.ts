...
import { createIngestLineageEmitter } from "./lib/openlineage";
import {
  clearFailure,
...
export async function runIngest(
  ...
  const lineage = createIngestLineageEmitter(env);
  const lineageMeta = { mode, dryRun, trigger };

  await lineage.emitStart(nowIso, lineageMeta);
  ...
  try {
    ...
    if (mode === "full-history") {
      ...
      const fetchedNewSummaries = await collectNewSummaries(
        fetchFn,
        config,
        state.lastIngestedLogId,
      );
    ...
    if (!dryRun) {
      console.log(
        `Emitting logs one at a time with per-send record cap ${config.pipelineBatchSize}`,
      );
    }
    ...
    await lineage.emitComplete(new Date().toISOString(), lineageMeta, result);
    return result;
  } catch (error) {
    await lineage.emitFail(new Date().toISOString(), lineageMeta, error);
    throw error;
  }
}
